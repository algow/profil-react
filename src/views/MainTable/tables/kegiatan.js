...
import { getPerkegiatan } from '../../../actions/filter';
...
class Kegiatan extends Component {
  ...
  componentDidMount(){
    getPerkegiatan(getUserData()['kode_kanwil']).then(res => {
        ...
        <Table.Body>
          {
            this.state.content.map((value, index) => (
              <Table.Row key={index}>
                ...
                <Table.Cell textAlign='center'>{value.kdgiat}</Table.Cell>
                <Table.Cell>{value.referensi[0].nmgiat}</Table.Cell>
