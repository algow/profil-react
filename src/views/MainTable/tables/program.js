...
import { getPerprogram } from '../../../actions/filter';
...
class Program extends Component {
  ...
  componentDidMount(){
    getPerprogram(getUserData()['kode_kanwil']).then(res => {
      ...
      <Table striped celled selectable sortable style={{ fontSize: 14 }} color='black'>
        <Table.Header>
          <Table.Row textAlign='center'>
            ...
            <Table.HeaderCell>Kode Progran</Table.HeaderCell>
            <Table.HeaderCell>Nama Progran</Table.HeaderCell>
        ...
        <Table.Body>
          {
            this.state.content.map((value, index) => (
              <Table.Row key={index}>
                ...
                <Table.Cell textAlign='center'>{value.kdprogram}</Table.Cell>
                <Table.Cell>{value.referensi[0].nmprogram}</Table.Cell>
