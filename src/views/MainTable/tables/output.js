...
import { getPeroutput } from '../../../actions/filter';
...
class Output extends Component {
  ...
  componentDidMount(){
    getPeroutput(getUserData()['kode_kanwil']).then(res => {
      ...
      <Table striped celled selectable sortable style={{ fontSize: 14 }} color='black'>
        <Table.Header>
          <Table.Row textAlign='center'>
            ...
            <Table.HeaderCell>Kode Output</Table.HeaderCell>
            <Table.HeaderCell>Nama Output</Table.HeaderCell>
        ...
        <Table.Body>
          {
            this.state.content.map((value, index) => (
              <Table.Row key={index}>
                ...
                <Table.Cell textAlign='center'>{value.kdoutput}</Table.Cell>
                <Table.Cell>{value.referensi[0].nmoutput}</Table.Cell>
