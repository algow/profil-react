...
import { getPersumberdana } from '../../../actions/filter';
import { getUserData } from '../../../helpers/user';

class SumberDana extends Component {
  constructor(props){
    super(props);
    this.state = {
      loading: true,
  ...
  componentDidMount(){
    getPersumberdana(getUserData()['kode_kanwil']).then(res => {
      ...
      <Table striped celled selectable sortable style={{ fontSize: 14 }} color='black'>
        <Table.Header>
          <Table.Row textAlign='center'>
            ...
            <Table.HeaderCell>Kode Sumber Dana</Table.HeaderCell>
            <Table.HeaderCell>Nama Sumber Dana</Table.HeaderCell>
        ...
        <Table.Body>
          {
            this.state.content.map((value, index) => (
              <Table.Row key={index}>
                ...
                <Table.Cell textAlign='center'>{value.kdsdana}</Table.Cell>
                <Table.Cell>{value.referensi[0].nmsdana}</Table.Cell>
